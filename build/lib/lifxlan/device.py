...
from .msgtypes import Acknowledgement, GetGroup, GetHostFirmware, GetInfo, GetLabel, GetLocation, GetPower, GetVersion, \
    GetWifiFirmware, GetWifiInfo, SERVICE_IDS, SetGroup, SetLabel, SetPower, StateGroup, StateHostFirmware, StateInfo, StateLabel, \
...
class Device(object):
    ...
    def set_group(self, label):
        ...
        if len(label) > 32:
            label = label[:32]
        self.req_with_ack(SetGroup, {"group": self.group, "label": label, "updated_at": int(time())})
